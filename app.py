...
# --- 1. 기본 설정 및 디자인 ---
st.set_page_config(page_title="패페 워싱봇 v2.1 (텍스트)", page_icon="✍️", layout="wide")
...
def call_ai(prompt):
    ...
    payload = {
        "model": "google/gemini-2.0-flash-lite-preview-02-05:free", # 무료 모델 중 가장 안정적
    ...
    try:
        ...
        if "error" in res_json:
            return f"에러: {res_json['error'].get('message', '한도 초과')}"
...
with col_in:
    ...
    user_guide = st.text_input("💡 AI 제작 가이드", placeholder="예: '신제품 강조', '반말 말고 존댓말로'")

    st.markdown('---')
    st.markdown('<div class="section-title">2. 작업 실행</div>', unsafe_allow_html=True)
    
    # 동준님 요청: 볼드/이모티콘 금지
    strict_rule = "\n[⚠️ 규칙: 이모티콘 사용 금지. 볼드(**)나 # 등 마크다운 형식 절대 금지. 텍스트만 출력.]"
...
with col_out:
    ...
    if b_thumb:
        with st.spinner("썸네일 구상 중..."):
            # 동준님 요청: 줄 구분 강화
            prompt = f"{strict_rule}\n\n5개를 번호를 매겨 제안해줘. 각 문구 사이엔 빈 줄을 한 칸씩 넣어줘.\n\n[밈]\n{meme_context}\n\n[가이드]\n{user_guide}\n\n[자료]\n{raw_text}"
            res = call_ai(prompt)
